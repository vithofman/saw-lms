...
class SAW_LMS_Redis_Driver implements SAW_LMS_Cache_Driver {
	...
	private function set_unique_prefix() {
		...
		if ( defined( 'SAW_LMS_REDIS_PREFIX' ) && ! empty( SAW_LMS_REDIS_PREFIX ) ) {
			...
			SAW_LMS_Logger::init()->debug(
				'Redis: Using manual prefix from wp-config.php',
				array(
					'prefix' => $this->prefix,
				)
			);
		...
		SAW_LMS_Logger::init()->info(
			'Redis: Auto-generated unique prefix',
			array(
				'prefix'   => $this->prefix,
				'site_url' => $site_url,
	...
	private function init_connection() {
		...
		try {
			...
			SAW_LMS_Logger::init()->debug(
				'Redis driver: Direct connection established',
				array(
					'host'       => $host,
					'port'       => $port,
					'database'   => $database,
			...
			SAW_LMS_Logger::init()->warning(
				'Redis driver: Connection failed',
				array(
					'error' => $e->getMessage(),
				)
			);
			...
			if ( $this->redis ) {
				...
				do {
					...
					if ( ++$iteration_count > $max_iterations ) {
						SAW_LMS_Logger::init()->warning(
							'Redis: Flush reached max iterations',
							array(
